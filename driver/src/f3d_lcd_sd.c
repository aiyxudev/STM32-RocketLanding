...
void f3d_lcd_drawBarclean(uint8_t x) {
  int i;
  uint16_t re[15] = {WHITE,WHITE,WHITE,WHITE,WHITE,WHITE,WHITE,WHITE,WHITE,WHITE,WHITE,WHITE,WHITE,WHITE,WHITE};
  f3d_lcd_setAddrWindow (x,20,x+15,140,MADCTLGRAPHICS);
  for (i = 0; i<120; i++) f3d_lcd_pushColor(re,15);
}


void f3d_lcd_drawBar(uint8_t x, uint8_t n, uint16_t color) {
  int i;
  uint16_t w[15];
  uint8_t h = (int)(n/7);
  for (i = 0; i < 15; i++) w[i] = color;
  f3d_lcd_setAddrWindow (x,80-h,x+15,80,MADCTLGRAPHICS);
  for (i = 0; i < h; i++) {
    f3d_lcd_pushColor(w,15);
  }
}

void f3d_lcd_drawBarneg(uint8_t x, uint8_t n, uint16_t color) {
  int i;
  uint16_t w[15];
  uint8_t h = (int)(n/7);
  for (i = 0; i < 15; i++) w[i] = color;
  f3d_lcd_setAddrWindow (x,80,x+15,140,MADCTLGRAPHICS);
  for (i = 0; i < h; i++) {
    f3d_lcd_pushColor(w,15);
  }
}
...
void f3d_lcd_clearRoc(uint8_t x, uint8_t y, uint16_t color) {
  int i;
  //for (i = 0; i < 360; i+=45) f3d_lcd_drawRoc(x+10*cosf(i*M_PI/180), y-10*sinf(i*M_PI/180), color);

  uint16_t re[21] = {color, color, color, color, color, color, color, color, color, color, color, color, color, color, color, color, color, color, color, color, color};
  f3d_lcd_setAddrWindow (x-10,y-10,x+10,y+19,MADCTLGRAPHICS);
  for (i = 0; i<20; i++) f3d_lcd_pushColor(re,21);
}


void f3d_lcd_drawObs(uint8_t x, uint8_t y, uint8_t r, uint16_t color) {
  int i;
  for (i = 0; i < 360; i+=30) f3d_lcd_drawRoc(x+r*cosf(i*M_PI/180), y-r*sinf(i*M_PI/180), color);
}


void f3d_lcd_drawPad(uint8_t x, uint8_t w, uint16_t color) {
  int i;
  uint16_t re[2*w+1];
  for (i = 0; i < 2*w+1; i++) re[i] = color;
  f3d_lcd_setAddrWindow (x-w,158,x+w,160,MADCTLGRAPHICS);
  for (i = 0; i<3; i++) f3d_lcd_pushColor(re,2*w+1);
}


void f3d_lcd_drawDot(uint8_t x, uint8_t y, uint16_t color) {
  f3d_lcd_drawPixel(x,y-2,color);
  f3d_lcd_drawPixel(x-1,y-1,color);
  f3d_lcd_drawPixel(x,y-1,color);
  f3d_lcd_drawPixel(x+1,y-1,color);
  f3d_lcd_drawPixel(x-2,y,color);
...
void f3d_lcd_drawRoc(uint8_t x, uint8_t y, uint16_t color) {
  f3d_lcd_drawPixel(x-1,y-2,color);
  f3d_lcd_drawPixel(x,y-2,color);
  f3d_lcd_drawPixel(x+1,y-2,color);
  f3d_lcd_drawPixel(x-2,y-1,color);
  f3d_lcd_drawPixel(x-1,y-1,color);
  f3d_lcd_drawPixel(x,y-1,color);
  f3d_lcd_drawPixel(x+1,y-1,color);
  f3d_lcd_drawPixel(x+2,y-1,color);
