...
void f3d_gyro_interface_init() {
  ...
  */
  ...
  // Alternate Functions Connect each pin to SPI1
...
void f3d_gyro_write(uint8_t* pBuffer, uint8_t WriteAddr, uint16_t NumByteToWrite) {
  ...
  if(NumByteToWrite> 1) WriteAddr |= (uint8_t) (0x40);
  //SET THE CS
  GYRO_CS_LOW();
  //SEND THE FIRST BYTE
  ...
  //De-assert the chip select (write it high).
  GYRO_CS_HIGH();
...
static uint8_t f3d_gyro_sendbyte(uint8_t byte) {
  ...
  // transmitter status prior to writing the byte to send to the transmitter register.
  while (SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_TXE) == RESET);
  SPI_SendData8(SPI1, byte);

  //poll to confirm that receiver has fully received the character prior to reading it
